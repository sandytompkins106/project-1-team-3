{% set config = {
    ...
    "target_table_name": "state_pm25_aqi_category",
    "months_back": 6,
    "parameter_name": "pm25"
} %}

-- Step 1: Pull raw PM2.5 readings per state for the last 6 months
...
-- Step 2: Average the readings per state
state_averages AS (
    SELECT
        state,
...
SELECT
    ...
    -- Rank states from most to least polluted
    RANK() OVER (ORDER BY avg_pm25 DESC) AS pollution_rank,

    NOW() AS updated_at

FROM state_averages
ORDER BY pollution_rank, avg_pm25 DESC, state;